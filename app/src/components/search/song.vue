    ...
    <div v-else>
      <mu-list style="font-size:14px">
        <mu-list-item @click.native="play(item)" :key="index" :class="{active:(index+1)%2!==0}" v-for="item,index in result">
          ...
          <div slot="right">
            <svg @click.stop="addToSonglist(item)" t="1494778069842" class="icon" style="padding-bottom:3px;font-size:20px;width: 1em; height: 1em;vertical-align: middle;fill: currentColor;overflow: hidden;"
