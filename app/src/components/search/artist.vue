  ...
  <div>
    ...
    <div v-else>
      ...
      <mu-row gutter :key="rowIndex" v-for="row, rowIndex in result">
        <mu-col @click.native="$store.commit('showInfo',{type:2,info:item})" width="50" v-if="index>=rowIndex*2&&index<(rowIndex+1)*2"
          :key="index" v-for="item, index in result">
          <mu-card>
            <mu-card-media :subTitle="item.albumSize+'张专辑'">
    ...
    methods: {
      ...
      getResult(key, searchType, offset = 0) {
        ...
        if (isNull(key)) {
          ...
          wyApi.search(key, searchType, offset, this.limit).then((data) => {
            if (data.code == 200) {
              if (data.result.hasOwnProperty('artists')) {
                this.result = data.result.artists
                this.key = key
      ...
      getMoreResult() {
        ...
        wyApi.search(this.key, this.searchType, this.moreTimes, this.limit).then((data) => {
          if (data.code == 200) {
            if (data.result.hasOwnProperty('artists')) {
              data.result.artists.forEach((item) => {
