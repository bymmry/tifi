...
<div class="music-player card" v-if="show">
	...
	<div class="music-box">
		<i-row :gutter="16">
			<i-col span="4" style="font-size:20px;">
				...
				<i-icon class="cursor" style="margin:0 1rem;" @click.native="playToggle" :type="!music.paused?'pause':'play'"></i-icon>
			...
			<i-col span="15">
				...
				<div style="line-height:25px">
					<mu-slider style="margin:0" :value="playProgress" @change="fastSeek"></mu-slider>
				</div>
			</i-col>
				...
				<i-row>
					...
					<i-col span="22" offset="1">
						...
						<div v-if="playlist" style="max-height:calc(300px - 25px - 2.6rem);overflow-y:auto">
							<div @click="$store.dispatch('playMusic', item)" class="playlist-content" :class="{'active':index==playlistActiveIndex}" v-for="item,index in playlist">
								<i-row>
									<i-col span="2">
										{{index+1}}
	...
	<div class="cursor" style="position:absolute;right:10px;top:0px;">
	...
	methods: {
		lastSong() {
			if (this.playlistActiveIndex > 0) {
				...
				this.$store.dispatch('playMusic', this.playlist[lastSongIndex])
			}
		},
		nextSong() {
			if (this.playlistActiveIndex < this.playlist.length - 1) {
				let nextSongIndex = this.playlistActiveIndex + 1
				this.$store.dispatch('playMusic', this.playlist[nextSongIndex])
			}
		},
		formatTime(time) {
		...
		fastSeek(time) {
			this.music.currentTime = time / 100 * this.totalTime
		},
		changeVolume(volume) {
		...
		audio: {
			inserted(el, binding, vnode) {
				...
				el.oncanplaythrough = function() {
					_this.music = el
					_this.$store.commit('setEl', el)
					_this.totalTime = el.duration
					if (_this.autoPlay) {
						_this.playToggle()
					}
